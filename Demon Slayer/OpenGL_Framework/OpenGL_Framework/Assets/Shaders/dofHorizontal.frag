...
void main()
{
	...
	col.rgb += texture(uTex, texcoord + vec2(- 4.0 * dofblur.x * uPixelSize.x, 0.0)).rgb * 0.06;
	col.rgb += texture(uTex, texcoord + vec2(- 3.0 * dofblur.x * uPixelSize.x, 0.0)).rgb * 0.09;
	col.rgb += texture(uTex, texcoord + vec2(- 2.0 * dofblur.x * uPixelSize.x, 0.0)).rgb * 0.12;
	col.rgb += texture(uTex, texcoord + vec2(-       dofblur.x * uPixelSize.x, 0.0)).rgb * 0.15;
